...
def getTitleAndAnswers(driver, index, isScore):
    title_and_answers = {}
    this_title_html = driver.find_elements_by_xpath('//li[@id="E' + str(index) + '"][@style="display: block;"]/table[1]/tbody[1]/tr[1]/td[1]/div[@class="ECnt"]')[0].get_attribute('innerHTML')
    ...
    for i in range(2, len(this_title_and_answers)):
        try:
            if isScore:
                title_and_answers[this_title_and_answers[i].split(u"、", 1)[0]] = this_title_and_answers[i].split(u"、", 1)[1]
            else:
                title_and_answers[this_title_and_answers[i].split(u"、", 1)[1]] = this_title_and_answers[i].split(u"、", 1)[0]
        except Exception, ex:
            print this_title_html
            if this_title_and_answers[i] != "" and u"、" in this_title_and_answers[i]:
                title_and_answers[this_title_and_answers[i].split(u"、", 1)[0]] = this_title_and_answers[i].split(u"、", 1)[0]
...
def yinyangWeb(list, name, age, sex, edu, metier, orgName):
    ...
    for index in range(1, totalQuestionCount+1):
        questionType = driver.find_elements_by_xpath('//li[@id="E' + str(index) + '"][@style="display: block;"]/table[1]/tbody[1]/tr[1]/td[1]/div[@class="ECnt"]/b[1]')[0].text.strip()
        ...
        if len(selections) == 0:
            ...
            for x in correctSelections:
                logger.info("正确答案：" + str(x))
            logger.info("选择了:" + str(selections))
            if multipleflag:
                for selection in selections:
                    driver.find_elements_by_xpath('//input[@type="checkbox"][@value="' + selection + str(index) + '"]')[0].click()
                driver.find_elements_by_xpath('//img[@id="Key_Next"]')[0].click()
            else:
                driver.find_elements_by_xpath('//input[@type="radio"][@value="' + selections[0] + str(index) + '"]')[0].click()
    ...
    confirm.accept()  # 接受
    time.sleep(1)
    screenshot(driver, "healthComputer", name, "pic")
    ...
    for i in range(1, totalQuestionCount+1):
        ...
        answers = driver.find_element_by_id("KWait" + str(1)).text
        if u"标准答案：" in answers:
            answers = answers.split(u"您的答案：")[1].split(u"标准答案：")[0].strip()
        else:
...
if __name__ == '__main__':
    conn = pymysql.connect(host="xxxxxx", user="xxxxx", password="xxx", database="questionnaire", charset="utf8")
    # 获取游标
    cur = conn.cursor()
